...
struct phys_device_props2_chain {
    ...
    void initialize_chain(AppInstance &inst, AppGpu &gpu , bool show_all_structs) noexcept {
        ...
        if ((gpu.CheckPhysicalDeviceExtensionIncluded(VK_KHR_DEPTH_STENCIL_RESOLVE_EXTENSION_NAME))
            && (gpu.api_version < VK_API_VERSION_1_2 || show_all_structs))
        ...
        if ((inst.CheckExtensionEnabled(VK_KHR_EXTERNAL_MEMORY_CAPABILITIES_EXTENSION_NAME)
         ...
         || inst.CheckExtensionEnabled(VK_KHR_EXTERNAL_FENCE_CAPABILITIES_EXTENSION_NAME))
            && (gpu.api_version < VK_API_VERSION_1_1 || show_all_structs))
        ...
        if ((gpu.CheckPhysicalDeviceExtensionIncluded(VK_EXT_INLINE_UNIFORM_BLOCK_EXTENSION_NAME))
            && (gpu.api_version < VK_API_VERSION_1_3 || show_all_structs))
...
#endif  // VK_ENABLE_BETA_EXTENSIONS
        if (gpu.api_version == VK_API_VERSION_1_1 || show_all_structs)
        ...
        if ((gpu.api_version >= VK_API_VERSION_1_2 || show_all_structs))
        ...
        if ((gpu.api_version >= VK_API_VERSION_1_3 || show_all_structs))
...
void setup_phys_device_props2_chain(VkPhysicalDeviceProperties2& start, std::unique_ptr<phys_device_props2_chain>& chain, AppInstance &inst,AppGpu &gpu,bool show_all_structs){
    chain = std::unique_ptr<phys_device_props2_chain>(new phys_device_props2_chain());
    chain->initialize_chain(inst,gpu,show_all_structs);
    start.pNext = chain->start_of_chain;
};

void chain_iterator_phys_device_props2(Printer &p, AppInstance &inst, AppGpu &gpu, bool show_all_structs, void * place) {
    while (place) {
...
struct phys_device_features2_chain {
    ...
    void initialize_chain(AppGpu &gpu , bool show_all_structs) noexcept {
...
void setup_phys_device_features2_chain(VkPhysicalDeviceFeatures2& start, std::unique_ptr<phys_device_features2_chain>& chain, AppGpu &gpu,bool show_all_structs){
    chain = std::unique_ptr<phys_device_features2_chain>(new phys_device_features2_chain());
    chain->initialize_chain(gpu,show_all_structs);
    start.pNext = chain->start_of_chain;
};

void chain_iterator_phys_device_features2(Printer &p, AppGpu &gpu, bool show_all_structs, void * place) {
    while (place) {
        struct VkBaseOutStructure *structure = (struct VkBaseOutStructure *)place;
        p.SetSubHeader();
