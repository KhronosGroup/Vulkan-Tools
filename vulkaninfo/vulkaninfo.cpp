...
void GpuDumpProps(Printer &p, AppGpu &gpu, bool show_all_structs) {
    ...
    if (gpu.inst.CheckExtensionEnabled(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME)) {
        ...
        chain_iterator_phys_device_props2(p, gpu.inst, gpu, show_all_structs, place);
...
void GpuDumpFeatures(Printer &p, AppGpu &gpu, bool show_all_structs) {
    p.SetHeader();
    DumpVkPhysicalDeviceFeatures(p, "VkPhysicalDeviceFeatures", gpu.features);
    p.AddNewline();
    if (gpu.inst.CheckExtensionEnabled(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME)) {
        void *place = gpu.features2.pNext;
        chain_iterator_phys_device_features2(p, gpu, show_all_structs, place);
    }
}
...
void DumpGpu(Printer &p, AppGpu &gpu, bool show_tooling_info, bool show_formats, bool show_all_structs) {
    ObjectWrapper obj_gpu(p, "GPU" + std::to_string(gpu.id));
    IndentWrapper indent(p);

    GpuDumpProps(p, gpu, show_all_structs);
    ...
    GpuDumpFeatures(p, gpu, show_all_structs);
...
const char *help_message_body =
    ...
    "[--show-all-structs] Include all available structs in pNext Chains.\n";
...
void print_usage(const std::string &executable_name) {
    ...
    std::cout << "    " << executable_name << " --show-all-structs\n";
...
util::vulkaninfo_optional<ParsedResults> parse_arguments(int argc, char **argv, std::string executable_name) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (strcmp(argv[i], "--show-all-structs") == 0) {
            results.show_all_structs = true;
...
void RunPrinter(Printer &p, ParsedResults parse_data, AppInstance &instance, std::vector<std::unique_ptr<AppGpu>> &gpus,
    ...
    if (parse_data.output_category == OutputCategory::summary) {
        ...
        for (auto &gpu : gpus) {
            DumpGpu(p, *(gpu.get()), parse_data.show_tool_props, parse_data.show_formats, parse_data.show_all_structs);
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        for (auto &phys_device : phys_devices) {
            gpus.push_back(std::unique_ptr<AppGpu>(new AppGpu(instance, gpu_counter++, phys_device, parse_data.show_all_structs)));
        }
